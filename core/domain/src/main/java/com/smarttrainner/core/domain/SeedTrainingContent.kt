...
object SeedTrainingContent {
    val exercises: List<Exercise> = listOf(
        ...
        exercise("bulgarian_split_squat", "불가리안 스플릿 스쿼트", MuscleGroup.LOWER_BODY, EquipmentType.DUMBBELL, DifficultyLevel.INTERMEDIATE, "bulgarian_split_squat", "뒤발을 벤치에 올려 한쪽 다리 힘과 균형을 강하게 쓰는 하체 운동입니다.", listOf("앞발 위치 잡기: 뒤발등을 벤치에 올리고 앞발은 뒤꿈치가 뜨지 않는 거리로 둡니다.", "몸통 세우기: 덤벨을 몸 옆에 두고 골반이 돌아가지 않게 복부를 조입니다.", "수직 하강: 앞무릎이 발끝 방향을 따라가게 하며 뒤무릎을 아래로 내립니다.", "앞발로 밀기: 앞발 전체로 바닥을 밀어 올라온 뒤 같은 반복 수를 반대쪽도 수행합니다."), listOf("앞무릎 통증이 있으면 보폭과 깊이를 줄이세요.", "균형이 흔들리면 덤벨 없이 시작하세요.", "뒤발로 벤치를 세게 밀어 튀어 오르지 마세요."), 3, 8..10, null, 90),
