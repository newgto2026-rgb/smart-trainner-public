...
object SeedTrainingContent {
    ...
    private val sortProfilesByExerciseId = mapOf(
        ...
        "hack_slide" to sortProfile(ExerciseMovementPattern.LEG_PRESS, popularityRank = 4, variantRank = 2),
    ...
    val exercises: List<Exercise> = listOf(
        ...
        exercise("hack_slide", "핵 슬라이드", MuscleGroup.LOWER_BODY, EquipmentType.MACHINE, DifficultyLevel.INTERMEDIATE, "hack_slide", "큰 고정 발판과 레일 위 슬라이딩 패드를 사용해 하체를 밀어내는 머신 운동입니다.", listOf("발판 세팅: 슬라이딩 패드에 등을 기대고 어깨 패드 아래에 어깨를 넣은 뒤 양발을 하나의 큰 발판에 올립니다.", "손잡이와 복압: 손잡이를 잡고 등과 엉덩이가 패드에서 뜨지 않게 복부에 힘을 줍니다.", "슬라이드 하강: 무릎이 발끝 방향을 따라가게 하며 패드가 레일을 따라 내려오도록 천천히 굽힙니다.", "발판 밀기: 발바닥 전체로 발판을 밀어 슬라이딩 패드를 올리되 무릎을 세게 잠그지 않습니다."), listOf("양발은 하나의 큰 발판에 올리고 프레임이나 보조 받침에 걸치지 마세요.", "허리나 엉덩이가 패드에서 뜨면 깊이를 줄이세요.", "무릎이 안쪽으로 모이지 않게 발끝 방향을 따라가세요."), 3, 10..12, null, 120),
