...
- `connectedDebugAndroidTest`는 Hilt에서 production `DataModule`을 제거하고 인메모리 `TrainingRepository`/`SessionRepository` fake를 사용한다. 테스트 초기화는 fake 상태만 reset하며 실제 앱 Room/DataStore를 clear하지 않는다.
